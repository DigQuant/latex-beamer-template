    ...
    \begin{block}{}
        \begin{center}
            \begin{tabular}{llll}
                ...
                \midrule
                内蒙古财经大学 & 从大模型应用到智能体实战项目 & 线上 & 25 年 12 月 1 日至 12 日 \\
                珠海科技学院 & Python 商务大数据分析实训 & 线下 & 25 年 12 月 6 日至 7 日 \\
                山西大学 & 大模型应用部署工程师实训 & 线下 & 26 年 1 月 8 日至 12 日 \\
                师资培训 & AI 赋能教学科研实战工作坊 & 线下 & 26 年 1 月 20 日至 23 日 \\
                韩山师范学院 & 人工智能金融应用实战 & 线上 & 26 年 1 月 26 日至 27 日 \\
                \bottomrule
            \end{tabular}
        \end{center}
    \end{block}
...
\begin{frame}[allowframebreaks]{内蒙古财经大学《从大模型应用到智能体实战项目》}
    ...
    \begin{alertblock}{问题反馈}
        \begin{itemize}
            \item 第一次线上上课，对时间把控与灵活性没有经验，同时已有课件内容过长，修改起来没有头绪。而且之前在南科大上课，学生接受程度高，上课时间需要更多高信息密度内容来填充，该经验导致误判已有学生水平，只能通过增加案例的方式作为上课时间的缓冲，因此第一版 ThesisTopicAssistant 提示词内容臃肿，并且不自信。
            \item 课件知识密度低，不适合拿来自学，新老师需要讲义辅助才能更快上手。
        \end{itemize}
    \end{alertblock}
\end{frame}
...
\begin{frame}[allowframebreaks]{山西大学《大数据大模型应用部署工程师实训》}
    ...
    \begin{alertblock}{问题反馈}
        \begin{itemize}
            ...
            \item 大模型相关案例使用到的第三方库接口变化巨大，例如  langchain 等经过大模型生成的代码接口过时，因此需要有 Python 项目管理工具，包括版本管理与依赖管理。最终采用的工具为 winget 与 uv，如果不采用的话，后续案例每次运行都要花大量时间教学生配置环境与解决环境依赖冲突。
            \item 学生电脑会出现各种各样的问题：系统文件缺失、驱动过时、空间不足、代理异常。例如 Python 的 micro 版本过小导致动态链接库不兼容，即仍处于 bug-fix 阶段；手动删除 C 盘系统文件或软件导致的动态链接库异常，极端情况甚至用到了 360；环境变量 PATH 内容过多触发长度限制，不断精简无效内容解决报错；硬盘清理与扩容，用到了 GeekUninstaller、WizTree 与 DiskGenius 等工具。
        \end{itemize}
    \end{alertblock}
\end{frame}
...
\begin{frame}[allowframebreaks]{韩山师范学院《人工智能金融应用实战》}
    \begin{block}{工作投入}
        \begin{itemize}
            \item \textbf{备课}：4 小时
            \item \textbf{授课}：线上 2 天 16 小时
        \end{itemize}
    \end{block}

    \begin{alertblock}{问题反馈：Ai-Lab}
        \begin{itemize}
            ...
            \item 学生作业无法批量下载、评分，如果使用网站工具模块来编写作业代码（1/64），最后只能在线查看，无法直接全部复制或下载，解决方法是抓包分析，手动解析 JSON。
        \end{itemize}
    \end{alertblock}
\end{frame}
